...
import com.yxk.impl.impPermissionService;
...
public class UserManagerController {
	...
	@Autowired
	private impPermissionService ips;
	...
	public @ResponseBody Result addUser(HttpServletRequest request)
	{
		...
		{
			result.setCode(0);
			result.setMessage("Url不能为空！请输入Url！");
			return result;
		}
		if(ips.getUser(name, url) > 0)
		{
			result.setCode(0);
	...
	{
		Result result = new Result();
		List<Permission> users = ips.getAll();
